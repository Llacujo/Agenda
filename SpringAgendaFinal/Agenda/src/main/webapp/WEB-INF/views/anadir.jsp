...
<body>
	<div class="container">
        ...
        <mvc:form modelAttribute="contacto" id="edit" action="/edition" method="post">
          ...
          <c:if test = "${empty contacto.telefonos}">
        	  <fieldset>
                    <p class="campo telefono">
                    <mvc:input path="telefonos[0].numero" type="tel" value="${telefono.numero}" /></p>
                    
                    <mvc:hidden path="telefonos[0].contacto.idContacto" value="${telefono.contacto.idContacto}" />
                    </fieldset>
               </c:if>     
            <!-- Direccion -->
            ...
            <!-- Provicncia -->
                 <fieldset>
                <p class="titulo provincia">Provincia:</p>
            </fieldset>
            <fieldset>
                <p class="titulo direccioncompleta">
                <mvc:select path="direccion.provincia.idProvincia">
                 <c:forEach var="provincia" items="${provincias}" varStatus="status">
                <mvc:option value="${provincia.idProvincia}">${provincia.provincia}</mvc:option>
            	</c:forEach>
            </mvc:select></p>
            </fieldset>
